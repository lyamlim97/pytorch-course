...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, patch_size: int = 16, embedding_dim: int = 768):
        ...
        self.flatten = nn.Flatten(
            start_dim=2, end_dim=3  # only flatten the feature map dimensions into a single vector
        )
...
class ViT(nn.Module):
    def __init__(
        ...
        num_classes=1000,
    ):  # generic number of classes (this can be adjusted)
        super().__init__()
        ...
        self.patch_embedding = PatchEmbedding(
            in_channels=num_channels, patch_size=patch_size, embedding_dim=embedding_dim
        )
